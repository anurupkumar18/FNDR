...
| MEM-07 | Memory finalization contract: invariants with tests | MEM | Anurup | Anurup+agent | W4 | 8 | P1 | MEM-02 | A stored memory must always carry provenance, a real embedding, and a lifecycle state or search and agents mislead | The rules are scattered across memory_quality.rs, validate_structured_memory_extraction, and the review lifecycle in HANDOFF.md | one assert_memory_contract test helper plus one test per invariant | `cd src-tauri && cargo test memory_contract` passes with one test per invariant in the WS5 plan section 4 | Test output |
