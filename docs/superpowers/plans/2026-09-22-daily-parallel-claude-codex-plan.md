...
- [ ] **Step 4:** Stop the app, run `make capture-baseline METRICS=/tmp/fndr-baseline.ndjson OUT=docs/evidence/W03/cap-05-context-baseline.md BEFORE_CONTEXT_P95=915` (per CAP-02's Makefile target). This produces both CAP-05's `capture.context_ms` before/after story (before = the documented AppleScript baseline in `docs/evidence/W01/tier2-merge-qa.md`, after = today's Accessibility-based numbers) and MEM-03's `mem.*` counters in one report.
