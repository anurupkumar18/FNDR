...
fn regenerate_search_aliases_basic(record: &MemoryRecord) -> Vec<String> {
    ...
    let aliases = crate::storage::generate_search_aliases_public(record);
    if !aliases.is_empty() {
        return aliases;
    }
...
fn memory_quality_scores(memory: &MemoryRecord) -> MemoryScoreBreakdown {
    MemoryScoreBreakdown {
        ...
        salience_concentration: crate::storage::salience_concentration_score(memory),
        topic_clarity: crate::storage::topic_clarity_score(memory),
        pollution_ratio: crate::storage::pollution_ratio_score(memory),
    }
}
...
pub async fn rebuild_memory_context_for_range(
    ...
    for index in 0..all.len() {
        ...
        rebuilt.insight_what_happened.clear();
        rebuilt.insight_why_mattered.clear();
        rebuilt.insight_what_changed.clear();
        rebuilt.insight_context_thread.clear();
        ...
        let different = rebuilt.memory_context != before.memory_context
            ...
            || rebuilt.insight_what_happened != before.insight_what_happened
            || rebuilt.insight_spans_json != before.insight_spans_json;
        if !different {
            continue;
        }
        changed += 1;
...
pub struct InsightBackfillReport {
    pub changed: usize,
    pub dry_run: bool,
}

/// Re-derive persisted insight columns and `embedding_text` for memories in a time range.
#[tauri::command]
pub async fn backfill_insight_layers_for_range(
    state: State<'_, Arc<AppState>>,
    start: i64,
    end: i64,
    dry_run: bool,
) -> Result<InsightBackfillReport, String> {
    if start > end {
        return Err("start must be <= end".to_string());
    }
    let mut all = state
