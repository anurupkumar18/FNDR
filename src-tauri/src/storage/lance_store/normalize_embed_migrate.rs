...
pub fn normalize_record_for_index(record: &MemoryRecord) -> MemoryRecord {
...
pub(super) fn infer_intent_analysis(record: &MemoryRecord) -> crate::storage::IntentAnalysis {
    ...
    let competing = ranked
        ...
        .skip(1)
        .take(2)
        .map(|(label, score)| crate::storage::IntentCandidate {
            label: (*label).to_string(),
            confidence: (*score / total).clamp(0.0, 1.0),
        })
        .collect::<Vec<_>>();

    crate::storage::IntentAnalysis {
...
pub(super) fn derive_structured_entities(
    ...
    for entity in dedup_strings(record.entities.clone()) {
        entities.push(crate::storage::ExtractedEntity {
            text: entity.clone(),
...
pub(super) fn derive_action_items(record: &MemoryRecord) -> Vec<crate::storage::MemoryActionItem> {
    let now = chrono::Utc::now().timestamp_millis();
...
pub(super) fn build_action_item(
    ...
    memory_id: &str,
    now: i64,
) -> crate::storage::MemoryActionItem {
    crate::storage::MemoryActionItem {
        kind: kind.to_string(),
...
> {
    ...
    .await?;
    let graph_nodes = open_or_create_named_table(
        &conn,
        &names,
        GRAPH_NODES_TABLE,
        Arc::new(insight_graph_node_schema()),
    )
    .await?;
    let graph_edges = open_or_create_named_table(
        &conn,
        &names,
        GRAPH_EDGES_TABLE,
        Arc::new(insight_graph_edge_schema()),
    )
    .await?;
...
pub(super) async fn ensure_memory_schema_columns(table: &Table) -> Result<(), lancedb::Error> {
    ...
    if !existing.contains("insight_what_happened") {
        transforms.push(("insight_what_happened".to_string(), "''".to_string()));
    }
    if !existing.contains("insight_why_mattered") {
        transforms.push(("insight_why_mattered".to_string(), "''".to_string()));
    }
    if !existing.contains("insight_what_changed") {
        transforms.push(("insight_what_changed".to_string(), "''".to_string()));
    }
    if !existing.contains("insight_context_thread") {
        transforms.push(("insight_context_thread".to_string(), "''".to_string()));
    }
    if !existing.contains("insight_spans_json") {
        transforms.push(("insight_spans_json".to_string(), "'{}'".to_string()));
    }
    if !existing.contains("insight_card_confidence") {
        transforms.push((
            "insight_card_confidence".to_string(),
            "CAST(0.0 AS FLOAT)".to_string(),
        ));
    }
